...
struct Game
{
	...
	std::condition_variable gameConditional;
...
struct PlayerPool
{
	...
	//std::unique_lock<std::mutex>* readyCountLock;
	std::condition_variable readyConditional;
	int* readyCount;

	std::mutex gunMutex;
	//std::unique_lock<std::mutex>* gunLock;
	std::condition_variable gunConditional;

	std::mutex playerFinishedMutex;
	//std::unique_lock<std::mutex>* playerFinishedLock;
	std::condition_variable playerFinishedConditional;

	int* playerFinishedCount;
...
void LogSync(LogSyncOperation operationToPerform)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	static std::mutex printingMutex;
	...
	while (currentGame->currentGameState == GameState::StillPlaying)
	{
		...
		switch (currentGame->currentGameState)
		{
			case GameState::StillPlaying:
				...
				///////////////////////////////////////////////////////////////////////////////////
				currentGame->gameConditional.notify_all();
				currentGame->gameConditional.wait((*currentGame->gameUniqueLock), [&]() {return currentGame->currentTurn == currentPlayer->type; });
				continue;
			case GameState::Won:
				///////////////////////////////////////////////////////////////////////////////////
...
void JoinGame(Player *currentPlayer, Game *currentGame)
{
	...
	if (currentGame->playerO == -1)
	{
		...
		currentGame->gameConditional.wait(gameUniqueLock, [&]() {return currentGame->playerCount == 2; });
	}
	else
	{
...
void PlayerThreadEntrypoint(Player*	currentPlayer)
{
	...
	//
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> gunLock(currentPlayer->playerPool->gunMutex); // create a lock from the gun mutex. this takes ownership of it and locks it.

	std::unique_lock<std::mutex> readyCountLock(currentPlayer->playerPool->readyMutex); // create a new lock from the same mutex used in main. this waits until the mutex is unlocked to aquire ownership of it. the mutex will be unlocked when main reaches the wait for the condition_variable
	*currentPlayer->playerPool->readyCount += 1;
	currentPlayer->playerPool->readyConditional.notify_all(); // notify main that there is another player that is ready.
	readyCountLock.unlock(); // unlock the mutex so that the next thread can take ownership of it.

	currentPlayer->playerPool->gunConditional.wait(gunLock, [&]() {return *currentPlayer->playerPool->readyCount == currentPlayer->playerPool->totalPlayerCount; }); // wait until main fires the gun
	gunLock.unlock();
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> playerFinishedLock(currentPlayer->playerPool->playerFinishedMutex); // create a new lock from the same mutex used in main. this waits until the mutex is unlocked to aquire ownership of it. the mutex will be unlocked when main reaches the wait for this locks condition_variable
	*currentPlayer->playerPool->playerFinishedCount += 1;
	currentPlayer->playerPool->playerFinishedConditional.notify_all(); // notify main to check if everyone is done
	playerFinishedLock.unlock(); // unlock the lock so main or the next thread can aquire it.
}
...
int main(int argc, char **argv)
{
	...
	int totalGameCount = atoi(argv[1]);
	// Total number of players that will be playing.
	int totalPlayerCount = atoi(argv[2]);
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> readyCountLock(poolOfPlayers.readyMutex); // create a lock from the mutex within the player pool. this locks the mutex and takes ownership of it.
	int readyCount = 0;

	std::unique_lock<std::mutex> playerFinishedLock(poolOfPlayers.playerFinishedMutex); // same thing here.
	int playerFinishedCount = 0;

	perPlayerData->playerPool = &poolOfPlayers;
	poolOfPlayers.perPlayerData = perPlayerData;
	poolOfPlayers.totalPlayerCount = totalPlayerCount;
	poolOfPlayers.readyCount = &readyCount;
	poolOfPlayers.playerFinishedCount = &playerFinishedCount;


	// Initialize each game
	for (int i = 0; i < totalGameCount; i++)
	{
		poolOfGames.perGameData[i].playerO = -1;
		poolOfGames.perGameData[i].playerX = -1;
		poolOfGames.perGameData[i].gameNumber = i + 1;
		poolOfGames.perGameData[i].currentTurn = PlayerType::X;
		poolOfGames.perGameData[i].currentGameState = GameState::StillPlaying;
		poolOfGames.perGameData[i].playerCount = 0;
		memset(poolOfGames.perGameData[i].gameBoard, 0, sizeof(poolOfGames.perGameData[i].gameBoard));
	}

	// Initialize each player
	for (int i = 0; i < totalPlayerCount; i++)
	{
		poolOfPlayers.perPlayerData[i].id = i;
		poolOfPlayers.perPlayerData[i].drawCount = 0;
		poolOfPlayers.perPlayerData[i].gamesPlayed = 0;
		poolOfPlayers.perPlayerData[i].loseCount = 0;
		poolOfPlayers.perPlayerData[i].winCount = 0;
		poolOfPlayers.perPlayerData[i].gamePool = &poolOfGames;
		poolOfPlayers.perPlayerData[i].playerPool = &poolOfPlayers;
		poolOfPlayers.perPlayerData[i].type = PlayerType::None;
	...
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all players to be ready. (starting line)
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.readyConditional.wait(readyCountLock, [&]() {return readyCount == totalPlayerCount; }); // wait until all players(threads) are ready before firing the gun
	readyCountLock.unlock(); // unlock it since we dont need it anymore

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Notify all waiting threads that they can start playing. (fire gun)
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> gunLock(poolOfPlayers.gunMutex); // wait until all other threads are done to aquire the gun mutex.
	poolOfPlayers.gunConditional.notify_all(); // fire the gun, starting all the threads
	gunLock.unlock(); // unlock it since we dont need it anymore


	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all detached player threads to complete.
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.playerFinishedConditional.wait(playerFinishedLock, [&]() {return playerFinishedCount == totalPlayerCount; }); // wait until all players are finished with their games
	playerFinishedLock.unlock(); //release it since we dont need it anymore
